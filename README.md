...
an `agent` is a harness -- an agent cli that runs the agentic loop in a sandbox. it comes in four kinds, each baking in the cli and how it authenticates. `opencode-openrouter` is the `opencode` cli with models served via openrouter (reads `OPENROUTER_API_KEY` from the env; write the model without the `openrouter/` prefix, it's added for you). `claude-cli-keyless` is the `claude` cli, keyless: your oauth creds (`~/.claude/.credentials.json`) are copied into the sandbox, and the model is a raw anthropic id. `codex-cli-keyless` is the `codex` cli, keyless: your chatgpt login (`~/.codex/auth.json`) is copied into an isolated `CODEX_HOME`, and the model is a raw openai id -- a codex login kept only in the os keyring has no file to copy, so switch codex to file credential storage first. `copilot-cli-keyless` is the `copilot` cli, keyless: a github token read from `COPILOT_GITHUB_TOKEN` in the env (an ambient `GH_TOKEN`/`GITHUB_TOKEN` is deliberately not forwarded), and the model is a copilot model id (e.g. `claude-sonnet-4.5`, `gpt-5`) -- copilot's default login lives in the os keychain, so mint the token first: macos `security find-generic-password -s copilot-cli -w`, or `gh auth token`. an agent can set `effort:` -- reasoning effort, mapped to claude's `--effort` (low|medium|high), codex's `model_reasoning_effort`, copilot's `--effort` (none|minimal|low|medium|high|xhigh|max), and opencode's `--variant` (provider-specific, e.g. high|max|minimal) respectively. all four harnesses log a machine-readable event stream to `stdout.log` (claude: stream-json; codex: `--json`; copilot: `--output-format json`; opencode: `--format json --thinking`), which feeds the live tool counter and the cost/token summary in each pair's `metadata.json` -- keyless codex and copilot are billed against the subscription, so they report tokens but no per-run dollar cost (copilot additionally records its premium-request count). every agent can run tasks; an agent you don't list in any suite simply never runs as one -- which is how you keep a stronger model around purely to score a weaker one (it must then be referenced by a `kind: agent` scorer, else it's flagged as an orphan). scorers come in two flavours. a file scorer (the default -- just omit `kind`) names an executable `score:` script; pats execs it directly and its shebang picks the interpreter and deps (e.g. `#!/usr/bin/env -S uv run --script` with inline pep-723 deps -- trampoline to python/spacy/whatever you like). it exits 0 and prints a float in `[0,1]` on its first stdout line, or `na` to skip (a non-zero exit is a failure). `${id}` in the `score:` path expands to the scorer's id, so a uniform layout reads `score: scorers/${id}.sh` (inside a flow mapping `{...}` quote it -- `"scorers/${id}.sh"` -- else the `}` closes the mapping). the other kind is `agent` (ask an agent to judge) -- **not implemented yet**; configuring one is rejected at config validation until it lands.
